...
function loadIFrame() {
    if (twitchApiLoaded && $(d).find('#chat-messages').length > 0) {
        ...
        d.styleSheets[0].insertRule('img[alt="🌝"] { background:url("https://static-cdn.jtvnw.net/emoticons/v1/25/1.0") no-repeat; width:0px; height:0px; padding-right:22px; padding-bottom:28px; background-size: 22px 28px;}', 0);
...
function addEmotes(spanObj, needsObserving) {
    ...
    wordsArray.each(function (i, word) {
        if (emotes[word] !== undefined) {
            var imgHtml = '<img class="twitch-emote style-scope yt-live-chat-text-message-renderer" src="' + template['small'].replace('{image_id}', emotes[word].image_id) + '" />';
